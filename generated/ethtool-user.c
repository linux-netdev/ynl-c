...
{
	if (op < 0 || op >= (int)YNL_ARRAY_SIZE(ethtool_op_strmap))
		return NULL;
...
{
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_udp_tunnel_type_strmap))
		return NULL;
...
{
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_stringset_strmap))
		return NULL;
...
static const char * const ethtool_module_fw_flash_status_strmap[] = {
	[0] = "started",
	[1] = "in-progress",
	[2] = "completed",
	[3] = "error",
...
{
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_module_fw_flash_status_strmap))
		return NULL;
	return ethtool_module_fw_flash_status_strmap[value];
}

static const char * const ethtool_c33_pse_ext_state_strmap[] = {
	[0] = "none",
	[1] = "error-condition",
	[2] = "mr-mps-valid",
	[3] = "mr-pse-enable",
...
static const char * const ethtool_hwtstamp_source_strmap[] = {
	[1] = "netdev",
	[2] = "phylib",
};
...
{
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_hwtstamp_source_strmap))
		return NULL;
	return ethtool_hwtstamp_source_strmap[value];
}

static const char * const ethtool_pse_event_strmap[] = {
	[0] = "pse-event-over-current",
	[1] = "pse-event-over-temp",
	[2] = "c33-pse-event-detection",
	[3] = "c33-pse-event-classification",
	[4] = "c33-pse-event-disconnection",
	[5] = "pse-event-over-budget",
	[6] = "pse-event-sw-pw-control-error",
};

const char *ethtool_pse_event_str(enum ethtool_pse_event value)
{
	value = ffs(value) - 1;
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_pse_event_strmap))
		return NULL;
	return ethtool_pse_event_strmap[value];
}

static const char * const ethtool_input_xfrm_strmap[] = {
	[0] = "sym-xor",
	[1] = "sym-or-xor",
};
...
{
	value = ffs(value) - 1;
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_input_xfrm_strmap))
		return NULL;
	return ethtool_input_xfrm_strmap[value];
}

static const char * const ethtool_rxfh_fields_strmap[] = {
	[1] = "l2da",
	[2] = "vlan",
	[3] = "l3-proto",
	[4] = "ip-src",
...
{
	value = ffs(value) - 1;
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(ethtool_rxfh_fields_strmap))
		return NULL;
...
const struct ynl_policy_nest ethtool_header_nest = {
...
const struct ynl_policy_nest ethtool_pause_stat_nest = {
...
const struct ynl_policy_attr ethtool_ts_hwtstamp_provider_policy[ETHTOOL_A_TS_HWTSTAMP_PROVIDER_MAX + 1] = {
	[ETHTOOL_A_TS_HWTSTAMP_PROVIDER_UNSPEC] = { .name = "unspec", .type = YNL_PT_REJECT, },
...
const struct ynl_policy_nest ethtool_cable_test_tdr_cfg_nest = {
...
const struct ynl_policy_attr ethtool_flow_policy[ETHTOOL_A_FLOW_MAX + 1] = {
	[ETHTOOL_A_FLOW_ETHER] = { .name = "ether", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_IP4] = { .name = "ip4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_IP6] = { .name = "ip6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_TCP4] = { .name = "tcp4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_TCP6] = { .name = "tcp6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_UDP4] = { .name = "udp4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_UDP6] = { .name = "udp6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_SCTP4] = { .name = "sctp4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_SCTP6] = { .name = "sctp6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_AH4] = { .name = "ah4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_AH6] = { .name = "ah6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_ESP4] = { .name = "esp4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_ESP6] = { .name = "esp6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_AH_ESP4] = { .name = "ah-esp4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_AH_ESP6] = { .name = "ah-esp6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU4] = { .name = "gtpu4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU6] = { .name = "gtpu6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPC4] = { .name = "gtpc4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPC6] = { .name = "gtpc6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPC_TEID4] = { .name = "gtpc-teid4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPC_TEID6] = { .name = "gtpc-teid6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_EH4] = { .name = "gtpu-eh4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_EH6] = { .name = "gtpu-eh6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_UL4] = { .name = "gtpu-ul4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_UL6] = { .name = "gtpu-ul6", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_DL4] = { .name = "gtpu-dl4", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FLOW_GTPU_DL6] = { .name = "gtpu-dl6", .type = YNL_PT_UINT, },
};

const struct ynl_policy_nest ethtool_flow_nest = {
	.max_attr = ETHTOOL_A_FLOW_MAX,
...
const struct ynl_policy_nest ethtool_mm_stat_nest = {
	.max_attr = ETHTOOL_A_MM_STAT_MAX,
	.table = ethtool_mm_stat_policy,
};

const struct ynl_policy_attr ethtool_mse_capabilities_policy[ETHTOOL_A_MSE_CAPABILITIES_MAX + 1] = {
	[ETHTOOL_A_MSE_CAPABILITIES_MAX_AVERAGE_MSE] = { .name = "max-average-mse", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MSE_CAPABILITIES_MAX_PEAK_MSE] = { .name = "max-peak-mse", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MSE_CAPABILITIES_REFRESH_RATE_PS] = { .name = "refresh-rate-ps", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MSE_CAPABILITIES_NUM_SYMBOLS] = { .name = "num-symbols", .type = YNL_PT_UINT, },
};

const struct ynl_policy_nest ethtool_mse_capabilities_nest = {
	.max_attr = ETHTOOL_A_MSE_CAPABILITIES_MAX,
	.table = ethtool_mse_capabilities_policy,
};

const struct ynl_policy_attr ethtool_mse_snapshot_policy[ETHTOOL_A_MSE_SNAPSHOT_MAX + 1] = {
	[ETHTOOL_A_MSE_SNAPSHOT_AVERAGE_MSE] = { .name = "average-mse", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MSE_SNAPSHOT_PEAK_MSE] = { .name = "peak-mse", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MSE_SNAPSHOT_WORST_PEAK_MSE] = { .name = "worst-peak-mse", .type = YNL_PT_UINT, },
};

const struct ynl_policy_nest ethtool_mse_snapshot_nest = {
	.max_attr = ETHTOOL_A_MSE_SNAPSHOT_MAX,
	.table = ethtool_mse_snapshot_policy,
};
...
const struct ynl_policy_nest ethtool_irq_moderation_nest = {
	.max_attr = ETHTOOL_A_IRQ_MODERATION_MAX,
	.table = ethtool_irq_moderation_policy,
};
...
const struct ynl_policy_nest ethtool_cable_result_nest = {
...
const struct ynl_policy_attr ethtool_fec_hist_policy[ETHTOOL_A_FEC_HIST_MAX + 1] = {
	[ETHTOOL_A_FEC_HIST_PAD] = { .name = "pad", .type = YNL_PT_IGNORE, },
	[ETHTOOL_A_FEC_HIST_BIN_LOW] = { .name = "bin-low", .type = YNL_PT_U32, },
	[ETHTOOL_A_FEC_HIST_BIN_HIGH] = { .name = "bin-high", .type = YNL_PT_U32, },
	[ETHTOOL_A_FEC_HIST_BIN_VAL] = { .name = "bin-val", .type = YNL_PT_UINT, },
	[ETHTOOL_A_FEC_HIST_BIN_VAL_PER_LANE] = { .name = "bin-val-per-lane", .type = YNL_PT_BINARY,},
};

const struct ynl_policy_nest ethtool_fec_hist_nest = {
	.max_attr = ETHTOOL_A_FEC_HIST_MAX,
	.table = ethtool_fec_hist_policy,
};
...
const struct ynl_policy_nest ethtool_bitset_bit_nest = {
...
const struct ynl_policy_nest ethtool_string_nest = {
...
const struct ynl_policy_nest ethtool_profile_nest = {
	.max_attr = ETHTOOL_A_PROFILE_MAX,
	.table = ethtool_profile_policy,
};
...
const struct ynl_policy_nest ethtool_cable_nest_nest = {
...
const struct ynl_policy_attr ethtool_fec_stat_policy[ETHTOOL_A_FEC_STAT_MAX + 1] = {
	...
	[ETHTOOL_A_FEC_STAT_HIST] = { .name = "hist", .type = YNL_PT_NEST, .nest = &ethtool_fec_hist_nest, },
};

const struct ynl_policy_nest ethtool_fec_stat_nest = {
...
const struct ynl_policy_nest ethtool_bitset_bits_nest = {
...
const struct ynl_policy_nest ethtool_tunnel_udp_table_nest = {
...
const struct ynl_policy_nest ethtool_stringsets_nest = {
...
const struct ynl_policy_nest ethtool_tunnel_udp_nest = {
...
const struct ynl_policy_nest ethtool_strset_nest = {
...
const struct ynl_policy_nest ethtool_linkinfo_nest = {
...
const struct ynl_policy_nest ethtool_linkmodes_nest = {
...
const struct ynl_policy_nest ethtool_debug_nest = {
...
const struct ynl_policy_nest ethtool_features_nest = {
...
const struct ynl_policy_nest ethtool_channels_nest = {
...
const struct ynl_policy_attr ethtool_coalesce_policy[ETHTOOL_A_COALESCE_MAX + 1] = {
	...
	[ETHTOOL_A_COALESCE_RX_PROFILE] = { .name = "rx-profile", .type = YNL_PT_NEST, .nest = &ethtool_profile_nest, },
	[ETHTOOL_A_COALESCE_TX_PROFILE] = { .name = "tx-profile", .type = YNL_PT_NEST, .nest = &ethtool_profile_nest, },
};

const struct ynl_policy_nest ethtool_coalesce_nest = {
...
const struct ynl_policy_nest ethtool_pause_nest = {
...
const struct ynl_policy_nest ethtool_eee_nest = {
...
const struct ynl_policy_attr ethtool_tsinfo_policy[ETHTOOL_A_TSINFO_MAX + 1] = {
	...
	[ETHTOOL_A_TSINFO_HWTSTAMP_SOURCE] = { .name = "hwtstamp-source", .type = YNL_PT_U32, },
	[ETHTOOL_A_TSINFO_HWTSTAMP_PHYINDEX] = { .name = "hwtstamp-phyindex", .type = YNL_PT_U32, },
};
...
const struct ynl_policy_nest ethtool_cable_test_nest = {
...
const struct ynl_policy_nest ethtool_cable_test_tdr_nest = {
...
const struct ynl_policy_nest ethtool_cable_test_tdr_ntf_nest = {
...
const struct ynl_policy_nest ethtool_tunnel_info_nest = {
...
const struct ynl_policy_nest ethtool_fec_nest = {
...
const struct ynl_policy_nest ethtool_module_eeprom_nest = {
...
const struct ynl_policy_nest ethtool_module_nest = {
...
const struct ynl_policy_attr ethtool_pse_policy[ETHTOOL_A_PSE_MAX + 1] = {
	...
	[ETHTOOL_A_C33_PSE_PW_CLASS] = { .name = "c33-pse-pw-class", .type = YNL_PT_U32, },
	[ETHTOOL_A_C33_PSE_ACTUAL_PW] = { .name = "c33-pse-actual-pw", .type = YNL_PT_U32, },
	[ETHTOOL_A_C33_PSE_EXT_STATE] = { .name = "c33-pse-ext-state", .type = YNL_PT_U32, },
	...
	[ETHTOOL_A_PSE_PW_D_ID] = { .name = "pse-pw-d-id", .type = YNL_PT_U32, },
	[ETHTOOL_A_PSE_PRIO_MAX] = { .name = "pse-prio-max", .type = YNL_PT_U32, },
	[ETHTOOL_A_PSE_PRIO] = { .name = "pse-prio", .type = YNL_PT_U32, },
};
...
const struct ynl_policy_attr ethtool_rss_policy[ETHTOOL_A_RSS_MAX + 1] = {
	...
	[ETHTOOL_A_RSS_INPUT_XFRM] = { .name = "input-xfrm", .type = YNL_PT_U32, },
	[ETHTOOL_A_RSS_START_CONTEXT] = { .name = "start-context", .type = YNL_PT_U32, },
	[ETHTOOL_A_RSS_FLOW_HASH] = { .name = "flow-hash", .type = YNL_PT_NEST, .nest = &ethtool_flow_nest, },
};

const struct ynl_policy_nest ethtool_rss_nest = {
...
const struct ynl_policy_nest ethtool_plca_nest = {
...
const struct ynl_policy_nest ethtool_mm_nest = {
...
const struct ynl_policy_attr ethtool_module_fw_flash_policy[ETHTOOL_A_MODULE_FW_FLASH_MAX + 1] = {
	...
	[ETHTOOL_A_MODULE_FW_FLASH_STATUS] = { .name = "status", .type = YNL_PT_U32, },
	[ETHTOOL_A_MODULE_FW_FLASH_STATUS_MSG] = { .name = "status-msg", .type = YNL_PT_NUL_STR, },
	[ETHTOOL_A_MODULE_FW_FLASH_DONE] = { .name = "done", .type = YNL_PT_UINT, },
	[ETHTOOL_A_MODULE_FW_FLASH_TOTAL] = { .name = "total", .type = YNL_PT_UINT, },
};

const struct ynl_policy_nest ethtool_module_fw_flash_nest = {
	.max_attr = ETHTOOL_A_MODULE_FW_FLASH_MAX,
	.table = ethtool_module_fw_flash_policy,
};
...
const struct ynl_policy_attr ethtool_phy_policy[ETHTOOL_A_PHY_MAX + 1] = {
	...
	[ETHTOOL_A_PHY_UPSTREAM_TYPE] = { .name = "upstream-type", .type = YNL_PT_U32, },
	[ETHTOOL_A_PHY_UPSTREAM_INDEX] = { .name = "upstream-index", .type = YNL_PT_U32, },
	[ETHTOOL_A_PHY_UPSTREAM_SFP_NAME] = { .name = "upstream-sfp-name", .type = YNL_PT_NUL_STR, },
	[ETHTOOL_A_PHY_DOWNSTREAM_SFP_NAME] = { .name = "downstream-sfp-name", .type = YNL_PT_NUL_STR, },
};

const struct ynl_policy_nest ethtool_phy_nest = {
...
const struct ynl_policy_attr ethtool_tsconfig_policy[ETHTOOL_A_TSCONFIG_MAX + 1] = {
	[ETHTOOL_A_TSCONFIG_UNSPEC] = { .name = "unspec", .type = YNL_PT_REJECT, },
...
const struct ynl_policy_attr ethtool_pse_ntf_policy[ETHTOOL_A_PSE_NTF_MAX + 1] = {
	[ETHTOOL_A_PSE_NTF_HEADER] = { .name = "header", .type = YNL_PT_NEST, .nest = &ethtool_header_nest, },
	[ETHTOOL_A_PSE_NTF_EVENTS] = { .name = "events", .type = YNL_PT_UINT, },
};

const struct ynl_policy_nest ethtool_pse_ntf_nest = {
	.max_attr = ETHTOOL_A_PSE_NTF_MAX,
	.table = ethtool_pse_ntf_policy,
};

const struct ynl_policy_attr ethtool_mse_policy[ETHTOOL_A_MSE_MAX + 1] = {
	[ETHTOOL_A_MSE_HEADER] = { .name = "header", .type = YNL_PT_NEST, .nest = &ethtool_header_nest, },
	[ETHTOOL_A_MSE_CAPABILITIES] = { .name = "capabilities", .type = YNL_PT_NEST, .nest = &ethtool_mse_capabilities_nest, },
	[ETHTOOL_A_MSE_CHANNEL_A] = { .name = "channel-a", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
	[ETHTOOL_A_MSE_CHANNEL_B] = { .name = "channel-b", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
	[ETHTOOL_A_MSE_CHANNEL_C] = { .name = "channel-c", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
	[ETHTOOL_A_MSE_CHANNEL_D] = { .name = "channel-d", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
	[ETHTOOL_A_MSE_WORST_CHANNEL] = { .name = "worst-channel", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
	[ETHTOOL_A_MSE_LINK] = { .name = "link", .type = YNL_PT_NEST, .nest = &ethtool_mse_snapshot_nest, },
};

const struct ynl_policy_nest ethtool_mse_nest = {
	.max_attr = ETHTOOL_A_MSE_MAX,
	.table = ethtool_mse_policy,
};
		       ...
		       struct ethtool_header *obj)
{
	...
	if (obj->_present.phy_index)
		ynl_attr_put_u32(nlh, ETHTOOL_A_HEADER_PHY_INDEX, obj->phy_index);
			 ...
			 const struct nlattr *nested)
{
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_HEADER_DEV_INDEX) {
			...
			dst->dev_index = ynl_attr_get_u32(attr);
			...
			dst->phy_index = ynl_attr_get_u32(attr);
		}
	}
			   ...
			   struct ethtool_pause_stat *obj)
{
	...
	if (obj->_present.tx_frames)
		ynl_attr_put_u64(nlh, ETHTOOL_A_PAUSE_STAT_TX_FRAMES, obj->tx_frames);
	if (obj->_present.rx_frames)
		ynl_attr_put_u64(nlh, ETHTOOL_A_PAUSE_STAT_RX_FRAMES, obj->rx_frames);
	ynl_attr_nest_end(nlh, nest);
			     ...
			     const struct nlattr *nested)
{
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_PAUSE_STAT_TX_FRAMES) {
			...
			dst->tx_frames = ynl_attr_get_u64(attr);
...
void ethtool_c33_pse_pw_limit_free(struct ethtool_c33_pse_pw_limit *obj)
{
}
...
int ethtool_c33_pse_pw_limit_parse(struct ynl_parse_arg *yarg,
				   const struct nlattr *nested)
{
	struct ethtool_c33_pse_pw_limit *dst = yarg->data;
	const struct nlattr *attr;
...
void ethtool_flow_free(struct ethtool_flow *obj)
{
}

int ethtool_flow_put(struct nlmsghdr *nlh, unsigned int attr_type,
		     struct ethtool_flow *obj)
{
	struct nlattr *nest;

	nest = ynl_attr_nest_start(nlh, attr_type);
	if (obj->_present.ether)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_ETHER, obj->ether);
	if (obj->_present.ip4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_IP4, obj->ip4);
	if (obj->_present.ip6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_IP6, obj->ip6);
	if (obj->_present.tcp4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_TCP4, obj->tcp4);
	if (obj->_present.tcp6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_TCP6, obj->tcp6);
	if (obj->_present.udp4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_UDP4, obj->udp4);
	if (obj->_present.udp6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_UDP6, obj->udp6);
	if (obj->_present.sctp4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_SCTP4, obj->sctp4);
	if (obj->_present.sctp6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_SCTP6, obj->sctp6);
	if (obj->_present.ah4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_AH4, obj->ah4);
	if (obj->_present.ah6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_AH6, obj->ah6);
	if (obj->_present.esp4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_ESP4, obj->esp4);
	if (obj->_present.esp6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_ESP6, obj->esp6);
	if (obj->_present.ah_esp4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_AH_ESP4, obj->ah_esp4);
	if (obj->_present.ah_esp6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_AH_ESP6, obj->ah_esp6);
	if (obj->_present.gtpu4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU4, obj->gtpu4);
	if (obj->_present.gtpu6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU6, obj->gtpu6);
	if (obj->_present.gtpc4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPC4, obj->gtpc4);
	if (obj->_present.gtpc6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPC6, obj->gtpc6);
	if (obj->_present.gtpc_teid4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPC_TEID4, obj->gtpc_teid4);
	if (obj->_present.gtpc_teid6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPC_TEID6, obj->gtpc_teid6);
	if (obj->_present.gtpu_eh4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_EH4, obj->gtpu_eh4);
	if (obj->_present.gtpu_eh6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_EH6, obj->gtpu_eh6);
	if (obj->_present.gtpu_ul4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_UL4, obj->gtpu_ul4);
	if (obj->_present.gtpu_ul6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_UL6, obj->gtpu_ul6);
	if (obj->_present.gtpu_dl4)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_DL4, obj->gtpu_dl4);
	if (obj->_present.gtpu_dl6)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FLOW_GTPU_DL6, obj->gtpu_dl6);
	ynl_attr_nest_end(nlh, nest);
...
{
	struct ethtool_flow *dst = yarg->data;
	const struct nlattr *attr;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_FLOW_ETHER) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ether = 1;
			dst->ether = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_IP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ip4 = 1;
			dst->ip4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_IP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ip6 = 1;
			dst->ip6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_TCP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.tcp4 = 1;
			dst->tcp4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_TCP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.tcp6 = 1;
			dst->tcp6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_UDP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.udp4 = 1;
			dst->udp4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_UDP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.udp6 = 1;
			dst->udp6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_SCTP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.sctp4 = 1;
			dst->sctp4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_SCTP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.sctp6 = 1;
			dst->sctp6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_AH4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ah4 = 1;
			dst->ah4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_AH6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ah6 = 1;
			dst->ah6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_ESP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.esp4 = 1;
			dst->esp4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_ESP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.esp6 = 1;
			dst->esp6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_AH_ESP4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ah_esp4 = 1;
			dst->ah_esp4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_AH_ESP6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ah_esp6 = 1;
			dst->ah_esp6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu4 = 1;
			dst->gtpu4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu6 = 1;
			dst->gtpu6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPC4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpc4 = 1;
			dst->gtpc4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPC6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpc6 = 1;
			dst->gtpc6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPC_TEID4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpc_teid4 = 1;
			dst->gtpc_teid4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPC_TEID6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpc_teid6 = 1;
			dst->gtpc_teid6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_EH4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_eh4 = 1;
			dst->gtpu_eh4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_EH6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_eh6 = 1;
			dst->gtpu_eh6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_UL4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_ul4 = 1;
			dst->gtpu_ul4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_UL6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_ul6 = 1;
			dst->gtpu_ul6 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_DL4) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_dl4 = 1;
			dst->gtpu_dl4 = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FLOW_GTPU_DL6) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.gtpu_dl6 = 1;
			dst->gtpu_dl6 = ynl_attr_get_uint(attr);
		}
	}
...
void ethtool_mse_capabilities_free(struct ethtool_mse_capabilities *obj)
{
}
...
int ethtool_mse_capabilities_parse(struct ynl_parse_arg *yarg,
				   const struct nlattr *nested)
{
	struct ethtool_mse_capabilities *dst = yarg->data;
	const struct nlattr *attr;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_MSE_CAPABILITIES_MAX_AVERAGE_MSE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.max_average_mse = 1;
			dst->max_average_mse = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MSE_CAPABILITIES_MAX_PEAK_MSE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.max_peak_mse = 1;
			dst->max_peak_mse = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MSE_CAPABILITIES_REFRESH_RATE_PS) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.refresh_rate_ps = 1;
			dst->refresh_rate_ps = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MSE_CAPABILITIES_NUM_SYMBOLS) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.num_symbols = 1;
			dst->num_symbols = ynl_attr_get_uint(attr);
		}
	}
...
void ethtool_mse_snapshot_free(struct ethtool_mse_snapshot *obj)
{
}
...
int ethtool_mse_snapshot_parse(struct ynl_parse_arg *yarg,
			       const struct nlattr *nested)
{
	struct ethtool_mse_snapshot *dst = yarg->data;
	const struct nlattr *attr;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_MSE_SNAPSHOT_AVERAGE_MSE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.average_mse = 1;
			dst->average_mse = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MSE_SNAPSHOT_PEAK_MSE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.peak_mse = 1;
			dst->peak_mse = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MSE_SNAPSHOT_WORST_PEAK_MSE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.worst_peak_mse = 1;
			dst->worst_peak_mse = ynl_attr_get_uint(attr);
		}
	}
...
int ethtool_irq_moderation_put(struct nlmsghdr *nlh, unsigned int attr_type,
			       struct ethtool_irq_moderation *obj)
{
	struct nlattr *nest;

	nest = ynl_attr_nest_start(nlh, attr_type);
	if (obj->_present.usec)
		ynl_attr_put_u32(nlh, ETHTOOL_A_IRQ_MODERATION_USEC, obj->usec);
	if (obj->_present.pkts)
		ynl_attr_put_u32(nlh, ETHTOOL_A_IRQ_MODERATION_PKTS, obj->pkts);
	if (obj->_present.comps)
		ynl_attr_put_u32(nlh, ETHTOOL_A_IRQ_MODERATION_COMPS, obj->comps);
	ynl_attr_nest_end(nlh, nest);
...
int ethtool_irq_moderation_parse(struct ynl_parse_arg *yarg,
				 const struct nlattr *nested)
{
	struct ethtool_irq_moderation *dst = yarg->data;
	const struct nlattr *attr;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_IRQ_MODERATION_USEC) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.usec = 1;
			dst->usec = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_IRQ_MODERATION_PKTS) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.pkts = 1;
			dst->pkts = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_IRQ_MODERATION_COMPS) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.comps = 1;
			dst->comps = ynl_attr_get_u32(attr);
		}
	}
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_CABLE_RESULT_PAIR) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.pair = 1;
			dst->pair = ynl_attr_get_u8(attr);
		} else if (type == ETHTOOL_A_CABLE_RESULT_CODE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.code = 1;
			dst->code = ynl_attr_get_u8(attr);
		} else if (type == ETHTOOL_A_CABLE_RESULT_SRC) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.src = 1;
			dst->src = ynl_attr_get_u32(attr);
		}
	}
...
int ethtool_fec_hist_put(struct nlmsghdr *nlh, unsigned int attr_type,
			 struct ethtool_fec_hist *obj)
{
	struct nlattr *nest;
	unsigned int i;

	nest = ynl_attr_nest_start(nlh, attr_type);
	if (obj->_present.bin_low)
		ynl_attr_put_u32(nlh, ETHTOOL_A_FEC_HIST_BIN_LOW, obj->bin_low);
	if (obj->_present.bin_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_FEC_HIST_BIN_HIGH, obj->bin_high);
	if (obj->_present.bin_val)
		ynl_attr_put_uint(nlh, ETHTOOL_A_FEC_HIST_BIN_VAL, obj->bin_val);
	if (obj->_count.bin_val_per_lane) {
		i = obj->_count.bin_val_per_lane * sizeof(__u64);
		ynl_attr_put(nlh, ETHTOOL_A_FEC_HIST_BIN_VAL_PER_LANE, obj->bin_val_per_lane, i);
	}
	ynl_attr_nest_end(nlh, nest);
...
int ethtool_fec_hist_parse(struct ynl_parse_arg *yarg,
			   const struct nlattr *nested)
{
	struct ethtool_fec_hist *dst = yarg->data;
	const struct nlattr *attr;
	unsigned int len;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_FEC_HIST_BIN_LOW) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.bin_low = 1;
			dst->bin_low = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_FEC_HIST_BIN_HIGH) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.bin_high = 1;
			dst->bin_high = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_FEC_HIST_BIN_VAL) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.bin_val = 1;
			dst->bin_val = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_FEC_HIST_BIN_VAL_PER_LANE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;

			len = ynl_attr_data_len(attr);
			dst->_count.bin_val_per_lane = len / sizeof(__u64);
			len = dst->_count.bin_val_per_lane * sizeof(__u64);
			dst->bin_val_per_lane = malloc(len);
			memcpy(dst->bin_val_per_lane, ynl_attr_data(attr), len);
		}
	}
...
void ethtool_profile_free(struct ethtool_profile *obj)
{
	unsigned int i;
...
int ethtool_profile_parse(struct ynl_parse_arg *yarg,
			  const struct nlattr *nested)
{
	struct ethtool_profile *dst = yarg->data;
...
void ethtool_fec_stat_free(struct ethtool_fec_stat *obj)
{
	...
	free(obj->uncorr);
	free(obj->corr_bits);
	for (i = 0; i < obj->_count.hist; i++)
		ethtool_fec_hist_free(&obj->hist[i]);
	free(obj->hist);
}
			   ...
			   const struct nlattr *nested)
{
	...
	if (n_hist) {
		dst->hist = calloc(n_hist, sizeof(*dst->hist));
		dst->_count.hist = n_hist;
		i = 0;
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_BITSET_NOMASK) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.size = 1;
			dst->size = ynl_attr_get_u32(attr);
			    ...
			    const struct nlattr *nested)
{
	...
	ynl_attr_for_each_nested(attr, nested) {
		...
		if (type == ETHTOOL_A_STRINGSET_ID) {
			...
			dst->count = ynl_attr_get_u32(attr);
			 ...
			 struct ethtool_linkinfo_set_req *req)
{
	...
	if (req->_present.port)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKINFO_PORT, req->port);
	if (req->_present.phyaddr)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKINFO_PHYADDR, req->phyaddr);
	if (req->_present.tp_mdix)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKINFO_TP_MDIX, req->tp_mdix);
	if (req->_present.tp_mdix_ctrl)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKINFO_TP_MDIX_CTRL, req->tp_mdix_ctrl);
	if (req->_present.transceiver)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKINFO_TRANSCEIVER, req->transceiver);
				    ...
				    struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_LINKMODES_HEADER) {
			...
			dst->autoneg = ynl_attr_get_u8(attr);
			  ...
			  struct ethtool_linkmodes_set_req *req)
{
	...
	if (req->_present.autoneg)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKMODES_AUTONEG, req->autoneg);
	...
	if (req->_present.speed)
		ynl_attr_put_u32(nlh, ETHTOOL_A_LINKMODES_SPEED, req->speed);
	if (req->_present.duplex)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKMODES_DUPLEX, req->duplex);
	if (req->_present.master_slave_cfg)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKMODES_MASTER_SLAVE_CFG, req->master_slave_cfg);
	if (req->_present.master_slave_state)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKMODES_MASTER_SLAVE_STATE, req->master_slave_state);
	if (req->_present.lanes)
		ynl_attr_put_u32(nlh, ETHTOOL_A_LINKMODES_LANES, req->lanes);
	if (req->_present.rate_matching)
		ynl_attr_put_u8(nlh, ETHTOOL_A_LINKMODES_RATE_MATCHING, req->rate_matching);
...
int ethtool_rings_set(struct ynl_sock *ys, struct ethtool_rings_set_req *req)
{
	...
	if (req->_present.rx_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_MAX, req->rx_max);
	if (req->_present.rx_mini_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_MINI_MAX, req->rx_mini_max);
	if (req->_present.rx_jumbo_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_JUMBO_MAX, req->rx_jumbo_max);
	if (req->_present.tx_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_TX_MAX, req->tx_max);
	if (req->_present.rx)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX, req->rx);
	if (req->_present.rx_mini)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_MINI, req->rx_mini);
	if (req->_present.rx_jumbo)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_JUMBO, req->rx_jumbo);
	if (req->_present.tx)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_TX, req->tx);
	if (req->_present.rx_buf_len)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_RX_BUF_LEN, req->rx_buf_len);
	if (req->_present.tcp_data_split)
		ynl_attr_put_u8(nlh, ETHTOOL_A_RINGS_TCP_DATA_SPLIT, req->tcp_data_split);
	if (req->_present.cqe_size)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RINGS_CQE_SIZE, req->cqe_size);
	if (req->_present.tx_push)
		ynl_attr_put_u8(nlh, ETHTOOL_A_RINGS_TX_PUSH, req->tx_push);
	if (req->_present.rx_push)
		ynl_attr_put_u8(nlh, ETHTOOL_A_RINGS_RX_PUSH, req->rx_push);
			 ...
			 struct ethtool_channels_set_req *req)
{
	...
	if (req->_present.header)
		ethtool_header_put(nlh, ETHTOOL_A_CHANNELS_HEADER, &req->header);
	if (req->_present.rx_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_RX_MAX, req->rx_max);
	if (req->_present.tx_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_TX_MAX, req->tx_max);
	if (req->_present.other_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_OTHER_MAX, req->other_max);
	if (req->_present.combined_max)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_COMBINED_MAX, req->combined_max);
	if (req->_present.rx_count)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_RX_COUNT, req->rx_count);
	if (req->_present.tx_count)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_TX_COUNT, req->tx_count);
	if (req->_present.other_count)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_OTHER_COUNT, req->other_count);
	if (req->_present.combined_count)
		ynl_attr_put_u32(nlh, ETHTOOL_A_CHANNELS_COMBINED_COUNT, req->combined_count);
...
{
	ethtool_header_free(&rsp->header);
	ethtool_profile_free(&rsp->rx_profile);
	ethtool_profile_free(&rsp->tx_profile);
	free(rsp);
}
...
void ethtool_coalesce_get_list_free(struct ethtool_coalesce_get_list *rsp)
{
	...
	while ((void *)next != YNL_LIST_END) {
		...
		ethtool_header_free(&rsp->obj.header);
		ethtool_profile_free(&rsp->obj.rx_profile);
		ethtool_profile_free(&rsp->obj.tx_profile);
		free(rsp);
	}
}
...
{
	ethtool_header_free(&req->header);
	ethtool_profile_free(&req->rx_profile);
	ethtool_profile_free(&req->tx_profile);
	free(req);
}
			 ...
			 struct ethtool_coalesce_set_req *req)
{
	...
	if (req->_present.rx_usecs)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_USECS, req->rx_usecs);
	if (req->_present.rx_max_frames)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_MAX_FRAMES, req->rx_max_frames);
	if (req->_present.rx_usecs_irq)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_USECS_IRQ, req->rx_usecs_irq);
	if (req->_present.rx_max_frames_irq)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_MAX_FRAMES_IRQ, req->rx_max_frames_irq);
	if (req->_present.tx_usecs)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_USECS, req->tx_usecs);
	if (req->_present.tx_max_frames)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_MAX_FRAMES, req->tx_max_frames);
	if (req->_present.tx_usecs_irq)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_USECS_IRQ, req->tx_usecs_irq);
	if (req->_present.tx_max_frames_irq)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_MAX_FRAMES_IRQ, req->tx_max_frames_irq);
	if (req->_present.stats_block_usecs)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_STATS_BLOCK_USECS, req->stats_block_usecs);
	if (req->_present.use_adaptive_rx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_COALESCE_USE_ADAPTIVE_RX, req->use_adaptive_rx);
	if (req->_present.use_adaptive_tx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_COALESCE_USE_ADAPTIVE_TX, req->use_adaptive_tx);
	if (req->_present.pkt_rate_low)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_PKT_RATE_LOW, req->pkt_rate_low);
	if (req->_present.rx_usecs_low)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_USECS_LOW, req->rx_usecs_low);
	if (req->_present.rx_max_frames_low)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_MAX_FRAMES_LOW, req->rx_max_frames_low);
	...
	if (req->_present.tx_max_frames_low)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_MAX_FRAMES_LOW, req->tx_max_frames_low);
	if (req->_present.pkt_rate_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_PKT_RATE_HIGH, req->pkt_rate_high);
	if (req->_present.rx_usecs_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_USECS_HIGH, req->rx_usecs_high);
	if (req->_present.rx_max_frames_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RX_MAX_FRAMES_HIGH, req->rx_max_frames_high);
	if (req->_present.tx_usecs_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_USECS_HIGH, req->tx_usecs_high);
	if (req->_present.tx_max_frames_high)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_MAX_FRAMES_HIGH, req->tx_max_frames_high);
	if (req->_present.rate_sample_interval)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_RATE_SAMPLE_INTERVAL, req->rate_sample_interval);
	if (req->_present.use_cqe_mode_tx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_COALESCE_USE_CQE_MODE_TX, req->use_cqe_mode_tx);
	if (req->_present.use_cqe_mode_rx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_COALESCE_USE_CQE_MODE_RX, req->use_cqe_mode_rx);
	if (req->_present.tx_aggr_max_bytes)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_AGGR_MAX_BYTES, req->tx_aggr_max_bytes);
	if (req->_present.tx_aggr_max_frames)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_AGGR_MAX_FRAMES, req->tx_aggr_max_frames);
	if (req->_present.tx_aggr_time_usecs)
		ynl_attr_put_u32(nlh, ETHTOOL_A_COALESCE_TX_AGGR_TIME_USECS, req->tx_aggr_time_usecs);
	if (req->_present.rx_profile)
		ethtool_profile_put(nlh, ETHTOOL_A_COALESCE_RX_PROFILE, &req->rx_profile);
	if (req->_present.tx_profile)
		ethtool_profile_put(nlh, ETHTOOL_A_COALESCE_TX_PROFILE, &req->tx_profile);
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_PAUSE_HEADER) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.rx = 1;
			dst->rx = ynl_attr_get_u8(attr);
		} else if (type == ETHTOOL_A_PAUSE_TX) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.tx = 1;
			dst->tx = ynl_attr_get_u8(attr);
			...
			dst->stats_src = ynl_attr_get_u32(attr);
		}
	}
...
int ethtool_pause_set(struct ynl_sock *ys, struct ethtool_pause_set_req *req)
{
	...
	if (req->_present.header)
		ethtool_header_put(nlh, ETHTOOL_A_PAUSE_HEADER, &req->header);
	if (req->_present.autoneg)
		ynl_attr_put_u8(nlh, ETHTOOL_A_PAUSE_AUTONEG, req->autoneg);
	if (req->_present.rx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_PAUSE_RX, req->rx);
	if (req->_present.tx)
		ynl_attr_put_u8(nlh, ETHTOOL_A_PAUSE_TX, req->tx);
	if (req->_present.stats)
		ethtool_pause_stat_put(nlh, ETHTOOL_A_PAUSE_STATS, &req->stats);
	if (req->_present.stats_src)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PAUSE_STATS_SRC, req->stats_src);
...
int ethtool_eee_set(struct ynl_sock *ys, struct ethtool_eee_set_req *req)
{
	...
	if (req->_present.active)
		ynl_attr_put_u8(nlh, ETHTOOL_A_EEE_ACTIVE, req->active);
	if (req->_present.enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_EEE_ENABLED, req->enabled);
	if (req->_present.tx_lpi_enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_EEE_TX_LPI_ENABLED, req->tx_lpi_enabled);
	if (req->_present.tx_lpi_timer)
		ynl_attr_put_u32(nlh, ETHTOOL_A_EEE_TX_LPI_TIMER, req->tx_lpi_timer);
				 ...
				 struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_TSINFO_HEADER) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.hwtstamp_source = 1;
			dst->hwtstamp_source = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_TSINFO_HWTSTAMP_PHYINDEX) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.hwtstamp_phyindex = 1;
			dst->hwtstamp_phyindex = ynl_attr_get_u32(attr);
		}
	}
			      ...
			      struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_FEC_HEADER) {
			...
			dst->auto_ = ynl_attr_get_u8(attr);
		} else if (type == ETHTOOL_A_FEC_ACTIVE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.active = 1;
			dst->active = ynl_attr_get_u32(attr);
...
int ethtool_fec_set(struct ynl_sock *ys, struct ethtool_fec_set_req *req)
{
	...
	if (req->_present.auto_)
		ynl_attr_put_u8(nlh, ETHTOOL_A_FEC_AUTO, req->auto_);
	if (req->_present.active)
		ynl_attr_put_u32(nlh, ETHTOOL_A_FEC_ACTIVE, req->active);
	if (req->_present.stats)
			  ...
			  struct ethtool_module_eeprom_get_req *req)
{
	...
	if (req->_present.offset)
		ynl_attr_put_u32(nlh, ETHTOOL_A_MODULE_EEPROM_OFFSET, req->offset);
	if (req->_present.length)
		ynl_attr_put_u32(nlh, ETHTOOL_A_MODULE_EEPROM_LENGTH, req->length);
	if (req->_present.page)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MODULE_EEPROM_PAGE, req->page);
	if (req->_present.bank)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MODULE_EEPROM_BANK, req->bank);
	if (req->_present.i2c_address)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MODULE_EEPROM_I2C_ADDRESS, req->i2c_address);
				 ...
				 struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_MODULE_HEADER) {
			...
			dst->power_mode_policy = ynl_attr_get_u8(attr);
		} else if (type == ETHTOOL_A_MODULE_POWER_MODE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.power_mode = 1;
			dst->power_mode = ynl_attr_get_u8(attr);
		}
	}
...
int ethtool_module_set(struct ynl_sock *ys, struct ethtool_module_set_req *req)
{
	...
	if (req->_present.power_mode_policy)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MODULE_POWER_MODE_POLICY, req->power_mode_policy);
	if (req->_present.power_mode)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MODULE_POWER_MODE, req->power_mode);
			      ...
			      struct ynl_parse_arg *yarg)
{
	...
	if (dst->c33_pse_pw_limit_ranges)
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_PSE_HEADER) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.c33_pse_pw_class = 1;
			dst->c33_pse_pw_class = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_C33_PSE_ACTUAL_PW) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.c33_pse_actual_pw = 1;
			dst->c33_pse_actual_pw = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_C33_PSE_EXT_STATE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.c33_pse_ext_state = 1;
			dst->c33_pse_ext_state = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_C33_PSE_EXT_SUBSTATE) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.c33_pse_ext_substate = 1;
			dst->c33_pse_ext_substate = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_C33_PSE_AVAIL_PW_LIMIT) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.c33_pse_avail_pw_limit = 1;
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.pse_pw_d_id = 1;
			dst->pse_pw_d_id = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_PSE_PRIO_MAX) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.pse_prio_max = 1;
			dst->pse_prio_max = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_PSE_PRIO) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.pse_prio = 1;
			dst->pse_prio = ynl_attr_get_u32(attr);
		}
	}
...
int ethtool_pse_set(struct ynl_sock *ys, struct ethtool_pse_set_req *req)
{
	...
	if (req->_present.c33_pse_avail_pw_limit)
		ynl_attr_put_u32(nlh, ETHTOOL_A_C33_PSE_AVAIL_PW_LIMIT, req->c33_pse_avail_pw_limit);
	if (req->_present.pse_prio)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PSE_PRIO, req->pse_prio);
...
void ethtool_rss_get_rsp_free(struct ethtool_rss_get_rsp *rsp)
{
	...
	free(rsp->indir);
	free(rsp->hkey);
	ethtool_flow_free(&rsp->flow_hash);
	free(rsp);
}
			      ...
			      struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_RSS_HEADER) {
			...
			dst->context = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_RSS_HFUNC) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.hfunc = 1;
			dst->hfunc = ynl_attr_get_u32(attr);
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.input_xfrm = 1;
			dst->input_xfrm = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_RSS_FLOW_HASH) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.flow_hash = 1;
...
ethtool_rss_get(struct ynl_sock *ys, struct ethtool_rss_get_req *req)
{
	...
	if (req->_present.context)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RSS_CONTEXT, req->context);
...
void ethtool_rss_get_list_free(struct ethtool_rss_get_list *rsp)
{
	...
	while ((void *)next != YNL_LIST_END) {
		...
		free(rsp->obj.indir);
		free(rsp->obj.hkey);
		ethtool_flow_free(&rsp->obj.flow_hash);
		free(rsp);
	}
}
...
ethtool_rss_get_dump(struct ynl_sock *ys, struct ethtool_rss_get_req_dump *req)
{
	...
	if (req->_present.header)
		ethtool_header_put(nlh, ETHTOOL_A_RSS_HEADER, &req->header);
	if (req->_present.start_context)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RSS_START_CONTEXT, req->start_context);
				   ...
				   struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_PLCA_HEADER) {
			...
			dst->version = ynl_attr_get_u16(attr);
			 ...
			 struct ethtool_plca_set_cfg_req *req)
{
	...
	if (req->_present.version)
		ynl_attr_put_u16(nlh, ETHTOOL_A_PLCA_VERSION, req->version);
	if (req->_present.enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_PLCA_ENABLED, req->enabled);
	if (req->_present.status)
		ynl_attr_put_u8(nlh, ETHTOOL_A_PLCA_STATUS, req->status);
	if (req->_present.node_cnt)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PLCA_NODE_CNT, req->node_cnt);
	if (req->_present.node_id)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PLCA_NODE_ID, req->node_id);
	if (req->_present.to_tmr)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PLCA_TO_TMR, req->to_tmr);
	if (req->_present.burst_cnt)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PLCA_BURST_CNT, req->burst_cnt);
	if (req->_present.burst_tmr)
		ynl_attr_put_u32(nlh, ETHTOOL_A_PLCA_BURST_TMR, req->burst_tmr);
...
int ethtool_mm_set(struct ynl_sock *ys, struct ethtool_mm_set_req *req)
{
	...
	if (req->_present.verify_enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MM_VERIFY_ENABLED, req->verify_enabled);
	if (req->_present.verify_time)
		ynl_attr_put_u32(nlh, ETHTOOL_A_MM_VERIFY_TIME, req->verify_time);
	if (req->_present.tx_enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MM_TX_ENABLED, req->tx_enabled);
	if (req->_present.pmac_enabled)
		ynl_attr_put_u8(nlh, ETHTOOL_A_MM_PMAC_ENABLED, req->pmac_enabled);
	if (req->_present.tx_min_frag_size)
		ynl_attr_put_u32(nlh, ETHTOOL_A_MM_TX_MIN_FRAG_SIZE, req->tx_min_frag_size);
			      ...
			      struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_PHY_HEADER) {
			...
			dst->upstream_type = ynl_attr_get_u32(attr);
		} else if (type == ETHTOOL_A_PHY_UPSTREAM_INDEX) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.upstream_index = 1;
			dst->upstream_index = ynl_attr_get_u32(attr);
...
void ethtool_rss_set_req_free(struct ethtool_rss_set_req *req)
{
	...
	free(req->indir);
	free(req->hkey);
	ethtool_flow_free(&req->flow_hash);
	free(req);
}
...
int ethtool_rss_set(struct ynl_sock *ys, struct ethtool_rss_set_req *req)
{
	...
	nlh = ynl_gemsg_start_req(ys, ys->family_id, ETHTOOL_MSG_RSS_SET, 1);
	ys->req_policy = &ethtool_rss_nest;
	ys->req_hdr_len = ys->family->hdr_len;
	...
	if (req->_present.context)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RSS_CONTEXT, req->context);
	if (req->_present.hfunc)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RSS_HFUNC, req->hfunc);
	if (req->_count.indir) {
		i = req->_count.indir * sizeof(__u32);
		ynl_attr_put(nlh, ETHTOOL_A_RSS_INDIR, req->indir, i);
	}
	if (req->_len.hkey)
		ynl_attr_put(nlh, ETHTOOL_A_RSS_HKEY, req->hkey, req->_len.hkey);
	if (req->_present.input_xfrm)
		ynl_attr_put_u32(nlh, ETHTOOL_A_RSS_INPUT_XFRM, req->input_xfrm);
	if (req->_present.flow_hash)
		ethtool_flow_put(nlh, ETHTOOL_A_RSS_FLOW_HASH, &req->flow_hash);
...
int ethtool_rss_create_act_rsp_parse(const struct nlmsghdr *nlh,
				     struct ynl_parse_arg *yarg)
{
	struct ethtool_rss_create_act_rsp *dst;
	const struct nlattr *attr;
	struct ynl_parse_arg parg;
	unsigned int len;
...
{
	struct ynl_req_state yrs = { .yarg = { .ys = ys, }, };
	struct ethtool_rss_create_act_rsp *rsp;
	struct nlmsghdr *nlh;
	unsigned int i;
	int err;

	nlh = ynl_gemsg_start_req(ys, ys->family_id, ETHTOOL_MSG_RSS_CREATE_ACT, 1);
	ys->req_policy = &ethtool_rss_nest;
	ys->req_hdr_len = ys->family->hdr_len;
	yrs.yarg.rsp_policy = &ethtool_rss_nest;
...
err_free:
	ethtool_rss_create_act_rsp_free(rsp);
	return NULL;
}
			   ...
			   struct ethtool_rss_delete_act_req *req)
{
	...
	nlh = ynl_gemsg_start_req(ys, ys->family_id, ETHTOOL_MSG_RSS_DELETE_ACT, 1);
	ys->req_policy = &ethtool_rss_nest;
	ys->req_hdr_len = ys->family->hdr_len;
...
{
	ethtool_header_free(&rsp->header);
	ethtool_mse_capabilities_free(&rsp->capabilities);
	ethtool_mse_snapshot_free(&rsp->channel_a);
	ethtool_mse_snapshot_free(&rsp->channel_b);
	ethtool_mse_snapshot_free(&rsp->channel_c);
	ethtool_mse_snapshot_free(&rsp->channel_d);
	ethtool_mse_snapshot_free(&rsp->worst_channel);
	ethtool_mse_snapshot_free(&rsp->link);
	free(rsp);
}
...
int ethtool_mse_get_rsp_parse(const struct nlmsghdr *nlh,
			      struct ynl_parse_arg *yarg)
{
	struct ethtool_mse_get_rsp *dst;
	const struct nlattr *attr;
	struct ynl_parse_arg parg;
...
{
	struct ynl_req_state yrs = { .yarg = { .ys = ys, }, };
	struct ethtool_mse_get_rsp *rsp;
	struct nlmsghdr *nlh;
	int err;

	nlh = ynl_gemsg_start_req(ys, ys->family_id, ETHTOOL_MSG_MSE_GET, 1);
	ys->req_policy = &ethtool_mse_nest;
	ys->req_hdr_len = ys->family->hdr_len;
	yrs.yarg.rsp_policy = &ethtool_mse_nest;

	if (req->_present.header)
		ethtool_header_put(nlh, ETHTOOL_A_MSE_HEADER, &req->header);
...
{
	struct ethtool_mse_get_list *next = rsp;
	...
	while ((void *)next != YNL_LIST_END) {
		...
		ethtool_header_free(&rsp->obj.header);
		ethtool_mse_capabilities_free(&rsp->obj.capabilities);
		ethtool_mse_snapshot_free(&rsp->obj.channel_a);
		ethtool_mse_snapshot_free(&rsp->obj.channel_b);
		ethtool_mse_snapshot_free(&rsp->obj.channel_c);
		ethtool_mse_snapshot_free(&rsp->obj.channel_d);
		ethtool_mse_snapshot_free(&rsp->obj.worst_channel);
		ethtool_mse_snapshot_free(&rsp->obj.link);
		free(rsp);
	}
}
...
ethtool_mse_get_dump(struct ynl_sock *ys, struct ethtool_mse_get_req_dump *req)
{
	...
	yds.yarg.data = NULL;
	yds.alloc_sz = sizeof(struct ethtool_mse_get_list);
	yds.cb = ethtool_mse_get_rsp_parse;
	yds.rsp_cmd = 54;

	nlh = ynl_gemsg_start_dump(ys, ys->family_id, ETHTOOL_MSG_MSE_GET, 1);
	ys->req_policy = &ethtool_mse_nest;
	ys->req_hdr_len = ys->family->hdr_len;
...
free_list:
	ethtool_mse_get_list_free(yds.first);
	return NULL;
}
...
int ethtool_module_fw_flash_ntf_rsp_parse(const struct nlmsghdr *nlh,
					  struct ynl_parse_arg *yarg)
{
	struct ethtool_module_fw_flash_ntf_rsp *dst;
	const struct nlattr *attr;
	struct ynl_parse_arg parg;
	unsigned int len;
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_MODULE_FW_FLASH_HEADER) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.status = 1;
			dst->status = ynl_attr_get_u32(attr);
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.done = 1;
			dst->done = ynl_attr_get_uint(attr);
		} else if (type == ETHTOOL_A_MODULE_FW_FLASH_TOTAL) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.total = 1;
			dst->total = ynl_attr_get_uint(attr);
		}
	}
...
int ethtool_pse_ntf_rsp_parse(const struct nlmsghdr *nlh,
			      struct ynl_parse_arg *yarg)
{
	struct ethtool_pse_ntf_rsp *dst;
	const struct nlattr *attr;
	struct ynl_parse_arg parg;
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == ETHTOOL_A_PSE_NTF_HEADER) {
			...
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.events = 1;
			dst->events = ynl_attr_get_uint(attr);
		}
	}
...
int ethtool_rss_delete_ntf_rsp_parse(const struct nlmsghdr *nlh,
				     struct ynl_parse_arg *yarg)
{
	struct ethtool_rss_delete_ntf_rsp *dst;
	const struct nlattr *attr;
	struct ynl_parse_arg parg;
...
static const struct ynl_ntf_info ethtool_ntf_info[] =  {
	...
	[ETHTOOL_MSG_MODULE_FW_FLASH_NTF] =  {
		.alloc_sz	= sizeof(struct ethtool_module_fw_flash_ntf),
	...
	[ETHTOOL_MSG_PSE_NTF] =  {
		.alloc_sz	= sizeof(struct ethtool_pse_ntf),
		.cb		= ethtool_pse_ntf_rsp_parse,
		.policy		= &ethtool_pse_ntf_nest,
		.free		= (void *)ethtool_pse_ntf_free,
	},
	[ETHTOOL_MSG_RSS_NTF] =  {
		.alloc_sz	= sizeof(struct ethtool_rss_get_ntf),
		.cb		= ethtool_rss_get_rsp_parse,
		.policy		= &ethtool_rss_nest,
		.free		= (void *)ethtool_rss_get_ntf_free,
	},
	[ETHTOOL_MSG_RSS_CREATE_NTF] =  {
		.alloc_sz	= sizeof(struct ethtool_rss_create_act_ntf),
		.cb		= ethtool_rss_create_act_rsp_parse,
		.policy		= &ethtool_rss_nest,
		.free		= (void *)ethtool_rss_create_act_ntf_free,
	},
	[ETHTOOL_MSG_RSS_DELETE_NTF] =  {
		.alloc_sz	= sizeof(struct ethtool_rss_delete_ntf),
		.cb		= ethtool_rss_delete_ntf_rsp_parse,
		.policy		= &ethtool_rss_nest,
		.free		= (void *)ethtool_rss_delete_ntf_free,
	},
};
...
const struct ynl_family ynl_ethtool_family =  {
	...
	.ntf_info_size	= YNL_ARRAY_SIZE(ethtool_ntf_info),
};
