...
{
	if (op < 0 || op >= (int)YNL_ARRAY_SIZE(fou_op_strmap))
		return NULL;
...
{
	if (value < 0 || value >= (int)YNL_ARRAY_SIZE(fou_encap_type_strmap))
		return NULL;
...
int fou_add(struct ynl_sock *ys, struct fou_add_req *req)
{
	...
	if (req->_present.ipproto)
		ynl_attr_put_u8(nlh, FOU_ATTR_IPPROTO, req->ipproto);
	if (req->_present.type)
		ynl_attr_put_u8(nlh, FOU_ATTR_TYPE, req->type);
	...
	if (req->_present.ifindex)
		ynl_attr_put_s32(nlh, FOU_ATTR_IFINDEX, req->ifindex);
...
int fou_get_rsp_parse(const struct nlmsghdr *nlh, struct ynl_parse_arg *yarg)
{
	...
	ynl_attr_for_each(attr, nlh, yarg->ys->family->hdr_len) {
		...
		if (type == FOU_ATTR_PORT) {
			...
			dst->ipproto = ynl_attr_get_u8(attr);
			...
			dst->local_v4 = ynl_attr_get_u32(attr);
			...
			dst->peer_port = ynl_attr_get_u16(attr);
		} else if (type == FOU_ATTR_IFINDEX) {
			if (ynl_attr_validate(yarg, attr))
				return YNL_PARSE_CB_ERROR;
			dst->_present.ifindex = 1;
			dst->ifindex = ynl_attr_get_s32(attr);
		}
	}
